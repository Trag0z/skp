...
int main( void )
{
	...
	// load texture
	SceUID fileID = sceIoOpen("app0:test.gxt", SCE_O_RDONLY, SCE_STM_RU);
	...
	SceUID texID;
	void* texPtr = graphicsAlloc(SCE_KERNEL_MEMBLOCK_TYPE_USER_RW_UNCACHE, dataSize, SCE_GXM_TEXTURE_ALIGNMENT, SCE_GXM_MEMORY_ATTRIB_READ, &texID);
...
void Update (void)
{
	...
	s_currentRotation += makeValue(ct.lx) * 0.1f;

    // We go from -1 to 1 in rendering.
    float aspectRatio = (float)DISPLAY_WIDTH / (float)DISPLAY_HEIGHT;

	Matrix4 rotation = Matrix4::rotationZ(s_currentRotation);
	Matrix4 scale = Matrix4::scale(Vector3((float)DISPLAY_HEIGHT / (float)DISPLAY_WIDTH, 1.0f, 1.0f));

	rotation = scale * rotation;

	float *temp = (float*)&rotation;

	memcpy(&s_wvpData[0], temp, 16 * 4);

    /*s_wvpData[ 0] = std::cosf(s_currentRotation) / aspectRatio;
	s_wvpData[1] = std::sinf(s_currentRotation);
	s_wvpData[ 2] = 0.0f;
	s_wvpData[ 3] = 0.0f;

	s_wvpData[4] = std::sinf(s_currentRotation) / aspectRatio;
	s_wvpData[5] = std::cosf(s_currentRotation);
	s_wvpData[ 6] = 0.0f;
	s_wvpData[ 7] = 0.0f;
...
/* ---------------------------------------------------------------------
	...
	returnCode = sceGxmShaderPatcherCreateVertexProgram( s_shaderPatcher, s_basicVertexProgramId, basicVertexAttributes, 2,
	...
	s_basicVertices = (BasicVertex *)graphicsAlloc( SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA_UNCACHE, 3*sizeof(BasicVertex), 4, SCE_GXM_MEMORY_ATTRIB_READ, &s_basicVerticesUId );
	s_basicIndices = (uint16_t *)graphicsAlloc( SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA_UNCACHE, 3*sizeof(uint16_t), 2, SCE_GXM_MEMORY_ATTRIB_READ, &s_basicIndiceUId );

	s_basicVertices[0].x = -0.5f;
	s_basicVertices[0].y = -0.5f;
	s_basicVertices[0].z = 0.0f;
	s_basicVertices[0].color = 0xff0000ff;
	s_basicVertices[1].x = 0.5f;
	s_basicVertices[1].y = -0.5f;
	s_basicVertices[1].z = 0.0f;
	s_basicVertices[1].color = 0xff00ff00;
	s_basicVertices[2].x = -0.5f;
	s_basicVertices[2].y = 0.5f;
	s_basicVertices[2].z = 0.0f;
	s_basicVertices[2].color = 0xffff0000;
...
/* -----------------------------------------------------------------
	...
	sceGxmDraw( s_context, SCE_GXM_PRIMITIVE_TRIANGLES, SCE_GXM_INDEX_FORMAT_U16, s_basicIndices, 3 );
