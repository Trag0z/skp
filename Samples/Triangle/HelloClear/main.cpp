...
void Update(void) {
    ...
    Matrix4 scale = Matrix4::scale(
        Vector3((float)DISPLAY_HEIGHT / (float)DISPLAY_WIDTH, 1.0f, 1.0f));
...
void createGxmData(void) {
    ...
    s_basicVertices = (BasicVertex *)graphicsAlloc(
        SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA_UNCACHE, 3 * sizeof(BasicVertex),
        4, SCE_GXM_MEMORY_ATTRIB_READ, &s_basicVerticesUId);
    s_basicIndices = (uint16_t *)graphicsAlloc(
        SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA_UNCACHE, 3 * sizeof(uint16_t), 2,
        SCE_GXM_MEMORY_ATTRIB_READ, &s_basicIndiceUId);
