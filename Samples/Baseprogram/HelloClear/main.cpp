...
static ClearVertex				*s_basicVertices			= NULL;
...
static const SceGxmProgramParameter *s_positionalParam = NULL;

static float s_lightingData;
static const SceGxmProgramParameter *s_lightParam = NULL;
...
int main( void )
{
	...
	sceTouchGetPanelInfo(SCE_TOUCH_PORT_FRONT, &s_touchInfo);
...
{
	SceTouchData tdf;
	int res = sceTouchRead(SCE_TOUCH_PORT_FRONT, &tdf, 6);
	...
	char text[512];
	if (tdf.reportNum > 0) {
		if (tdf.report[0].id != s_lastTouchId) {
			s_toggleState = !s_toggleState;
			s_lastTouchId = tdf.report[0].id;
		}
		if (s_toggleState)
			s_lightingData = 1.0f;
		else
		...
		sprintf(text, "X: %d, Y: %d, ID: %d", tdf.report[0].x, tdf.report[0].y, tdf.report[0].id);
	
		s_positionalData[0] = 2.0f * tdf.report[0].x / s_touchInfo.maxAaX - 1.0f;
		s_positionalData[1] = -2.0f * tdf.report[0].y / s_touchInfo.maxAaY + 1.0f;
	}
	else {
		sprintf(text, "No touch");
		s_lightingData = 0.0f;
	}
	sceDbgFontPrint(20, 20, 0xffffffff, (const SceChar8*)&(text[0]));
...
/* ---------------------------------------------------------------------
	...
	returnCode = sceGxmShaderPatcherCreateVertexProgram( s_shaderPatcher, s_basicVertexProgramId, clearVertexAttributes, 1,
														 clearVertexStreams, 1, &s_basicVertexProgram );
	SCE_DBG_ALWAYS_ASSERT( returnCode == SCE_OK );
	...
	s_positionalParam = sceGxmProgramFindParameterByName( basicProgram, "translation" );
	SCE_DBG_ALWAYS_ASSERT( s_positionalParam && ( sceGxmProgramParameterGetCategory( s_positionalParam ) == SCE_GXM_PARAMETER_CATEGORY_UNIFORM ) );
    s_lightParam = sceGxmProgramFindParameterByName(sceGxmShaderPatcherGetProgramFromId(s_basicFragmentProgramId), "lighting");
    SCE_DBG_ALWAYS_ASSERT( s_lightParam && ( sceGxmProgramParameterGetCategory( s_lightParam ) == SCE_GXM_PARAMETER_CATEGORY_UNIFORM ) );

	/* create shaded triangle vertex/index data */
	s_basicVertices = (ClearVertex *)graphicsAlloc( SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA_UNCACHE, 4*sizeof(ClearVertex), 4, SCE_GXM_MEMORY_ATTRIB_READ, &s_basicVerticesUId );
	s_basicIndices = (uint16_t *)graphicsAlloc( SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA_UNCACHE, 6*sizeof(uint16_t), 2, SCE_GXM_MEMORY_ATTRIB_READ, &s_basicIndiceUId );

	s_basicVertices[0].x = -0.1f;
	s_basicVertices[0].y = -0.1f;
	
	s_basicVertices[1].x = -0.1f;
	s_basicVertices[1].y = 0.1f;

    s_basicVertices[2].x = 0.1f;
	s_basicVertices[2].y = -0.1f;

    s_basicVertices[3].x = 0.1f;
	s_basicVertices[3].y = 0.1f;
...
/* -----------------------------------------------------------------
	...
	sceGxmSetUniformDataF( vertexDefaultBuffer, s_positionalParam, 0, 2, s_positionalData );

    void *fragmentDefaultBuffer;
    sceGxmReserveFragmentDefaultUniformBuffer(s_context, &fragmentDefaultBuffer);
    sceGxmSetUniformDataF( fragmentDefaultBuffer, s_lightParam, 0, 1, &s_lightingData );

	/* draw the spinning triangle */
	sceGxmSetVertexStream( s_context, 0, s_basicVertices );
	sceGxmDraw( s_context, SCE_GXM_PRIMITIVE_TRIANGLES, SCE_GXM_INDEX_FORMAT_U16, s_basicIndices, 6 );
